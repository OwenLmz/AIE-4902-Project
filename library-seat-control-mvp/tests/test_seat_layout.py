...
class SeatLayoutTest(unittest.TestCase):
    ...
    def test_unknown_status_does_not_crash_and_shows_unknown_label(self) -> None:
        svg = build_svg_seat_layout([
            {"seat_id": "A01", "status": "BROKEN", "roi_x1": 0, "roi_y1": 0, "roi_x2": 10, "roi_y2": 10}
        ])
        self.assertIn("未知状态", svg)

    def test_seat_id_is_html_escaped(self) -> None:
        svg = build_svg_seat_layout([
            {"seat_id": "<A&1>", "status": "free", "roi_x1": 0, "roi_y1": 0, "roi_x2": 10, "roi_y2": 10}
        ])
    ...
    def test_different_camera_ids_are_not_in_same_group(self) -> None:
        groups = group_seats_by_layout([
            {"seat_id": "A01", "floor": "3", "zone": "A", "camera_id": "CAM-1", "roi_x1": 0, "roi_y1": 0, "roi_x2": 10, "roi_y2": 10},
            {"seat_id": "A02", "floor": "3", "zone": "A", "camera_id": "CAM-2", "roi_x1": 0, "roi_y1": 0, "roi_x2": 10, "roi_y2": 10},
        ])
    ...
    def test_selected_seat_id_creates_highlight_style(self) -> None:
        svg = build_svg_seat_layout([
            {"seat_id": "A01", "status": "free", "roi_x1": 0, "roi_y1": 0, "roi_x2": 10, "roi_y2": 10}
    ...
    def test_free_seat_has_stronger_visual_weight(self) -> None:
        svg = build_svg_seat_layout([
            {"seat_id": "A01", "status": "free", "roi_x1": 0, "roi_y1": 0, "roi_x2": 10, "roi_y2": 10},
            {"seat_id": "A02", "status": "occupied", "roi_x1": 20, "roi_y1": 0, "roi_x2": 30, "roi_y2": 10},
        ])
    ...
    def test_recommended_seat_has_star_marker_class(self) -> None:
        svg = build_svg_seat_layout([
            {"seat_id": "F3-A04", "status": "free", "roi_x1": 0, "roi_y1": 0, "roi_x2": 10, "roi_y2": 10}
    ...
    def test_legacy_polygon_still_renders_for_compatibility(self) -> None:
        svg = build_svg_seat_layout([
